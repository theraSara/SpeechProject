...
class NCMOpenMax(nn.Module):
    """
    This is an implementation of the openMAX algorithm for streaming learning.
    """
    ...
    def __init__(self, backbone=None, cuda=True):
        ...
        # feature extraction backbone: map the input to embedding space 
        ...
        if backbone is not None:
            ...
            if not hasattr(self.backbone.criterion, 'distance'):
                self.backbone.criterion.distance = 'euclidean' # euclidean or cosine
...
#         print(x.size())
    ...
    def fit_batch_offline(self, x, class_list, tailsize=5):
        ...
        :param item_ix:
        :param x: a torch tensor of the input support data (must be a vector)
        ...
        # zq[i,j]: embedding of support example j from category i 
        # [no category, support shot, embedding dimension]
        zq = self.backbone.get_embeddings(xx)
        zq = zq.view(self.num_classes, n_support, zq.size(-1))
        # mean embeddings for each category 
        z_proto = zq.mean(1)
...
#                 print(f"W Score: {categoryid}: {wscore}")
                ...
                # based on distance directly, not probabilities 
                modified_fc8_score = channel_distance * wscore
                openmax_class_score += [modified_fc8_score]
                # unknown score = sum of unknown parts of all classes: d_i-w_i*d_i
