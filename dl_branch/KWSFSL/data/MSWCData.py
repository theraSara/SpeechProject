...
class MSWCDataset:
    def __init__(self, data_dir, MSWCtype, cuda, args):
        ...
        # speed perturbation
        self.use_speed_perturb = args.get('speed_perturb', False)
        self.speed_factors = args.get('speed_factors', [0.9, 1.0, 1.1])
    ...
    def get_transform_dataset(self, file_dict, classes, filters=None):
        ...
        transforms = compose([
                ...
                partial(self.speed_perturb, 'data'),
    ...
    def speed_perturb(self, k, d):
        if not self.use_speed_perturb:
            return d
        factor = np.random.choice(self.speed_factors)
        if factor != 1.0:
            audio = d[k]
            # resample to factor*sample_rate then treat as sample_rate — equivalent to time stretch
            new_sr = int(self.sample_rate * factor)
            d[k] = torchaudio.functional.resample(audio, new_sr, self.sample_rate)
        return d
